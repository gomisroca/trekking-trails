    ...
    <UCard class="w-[600px]">
      <form class="p-2 flex flex-col space-y-6" @submit.prevent="onSubmit">
        ...
        <UFormGroup
          ...
          name="categories"
          class="uppercase font-semibold"
          :error="!state.categories && 'Required'"
        >
          <UInput v-model="state.categories" />
        </UFormGroup>
        ...
        <UFormGroup
          ...
          name="content"
          class="uppercase font-semibold"
          :error="!state.content && 'Required'"
        >
          <UTextarea autoresize v-model="state.content" />
        </UFormGroup>
        ...
        <UFormGroup label="Cover Pictures" class="uppercase font-semibold">
          <input
            ...
            type="file"
            multiple
            @change="handleFileUpload($event, 'covers')"
          />
        </UFormGroup>
        ...
        <UButton type="submit" class="uppercase font-bold dark:text-secondary"
          >Upload Post</UButton
        >
      </form>
      <div class="m-auto">
        <span v-if="success" class="text-primary font-medium">{{
          success
        }}</span>
...
async function onSubmit() {
  try {
    ...
    const coverFiles = covers.value?.files;
    const galleryFiles = gallery.value?.files;

    if (coverFiles) {
      Array.from(coverFiles).forEach((file) => {
